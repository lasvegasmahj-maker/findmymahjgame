...
- No server-side event logging either: `app/api/ask/route.ts` answers questions but persists only anonymized rules-gap telemetry (topic summaries with emails and digits stripped, added with the rules capability); zero-result searches and directory questions remain unlogged.
