...
| --- | --- | --- | --- |
| `blank-tiles` | 1 | "They come with many sets as spares to replace a lost tile. Some tables use them as a house rule, usually (more in the panel transcript) | These two sentences follow Mahj Life 279's opening sequence idea-for-idea and in the same order (many sets / replace lost tiles; some groups adopt them as a (more in the panel transcript) |
| `calling-discard-house-note` | 1 | knowledge.ts line 504, house_note on the calling-discard entry: "The card closes the calling window once the (more in the panel transcript) | Two wording risks, neither a wrong rule. (1) Attribution: the note twice credits "the card" for the calling-window rule, but the documented League source for (more in the panel transcript) |
| `calling-discard/corpus-missing-house-note` | 1 | corpus.json entry "calling-discard" has no house_note field, but (more in the panel transcript) | Process risk, not a rule error: reviewers working only from corpus.json cannot see (and therefore cannot check) the house_note text, which is exactly where the (more in the panel transcript) |
| `calling-discard/house-note-card-attribution` | 1 | house_note (knowledge.ts line 503): "The card closes the calling window once the player next in turn has (more in the panel transcript) | The rule itself is League law and verified. Two scope issues: (1) Attribution : the closing rule as worded is published in the League's rulebook Mah Jongg Made (more in the panel transcript) |
| `calling-discard/quint-sextet-scope` | 1 | Cross-entry scope tension: calling-discard (owner-approved) says a call is allowed for "an exposed group of 3 (more in the panel transcript) | Not a contradiction and not a wrong statement: the owner_question entry claims no rule, and calling-for-exposure still says '3 or more'. But the owner-approved (more in the panel transcript) |
| `calling-discard/varies-by-house-flag` | 1 | knowledge.ts line 500-502: varies_by_house: true with house_note "The card closes the calling window once the (more in the panel transcript) | Classification risk, not a wrong statement. The calling window is League law (the note itself says the card sets it); what varies is only how strictly tables (more in the panel transcript) |
| `calling-for-exposure` | 4 | (1) "you may fix a mistake in that exposure only until you discard"; (2) "a Pung, a Kong, or a larger group" | No wrong League statement, but two scope risks. (1) The correction window is slightly overstated: per Mahj Life 177 the exposure may be corrected only if the (more in the panel transcript) |
| `calling-for-exposure/joker-exchange-lock` | 1 | "you may fix a mistake in that exposure only until you discard" | Incomplete rather than wrong: the League added in 2025 that a joker exchange completed after the call also freezes the exposure for that turn. A player who (more in the panel transcript) |
| `calling-for-mahjong` | 2 | "the chance to call ends once the next player has drawn and racked a tile" | Classification (standard_nmjl_rule) is right and every substantive rule in the answer is League-supported, but the calling-window clause is narrower than the (more in the panel transcript) |
| `calling-quints-sextets` | 1 | Corpus version (source owner_question): "Calling for a Pung or a Kong follows the usual rule: the tile must (more in the panel transcript) | Two problems with the corpus snapshot. (1) An owner_question entry is contractually limited to saying the instructor is confirming, yet this one also states a (more in the panel transcript) |
| `calling-quints-sextets-source-divergence` | 1 | corpus.json answer: "Whether a discard may be called to complete a quint or a sextet is a detail our (more in the panel transcript) | The corpus and the shipped TypeScript source disagree on this entry's answer, source, evidence, classification and confidence. The TypeScript "Yes" answer is (more in the panel transcript) |
| `calling-quints-sextets-vs-exposure` | 1 | calling-quints-sextets: "Whether a discard may be called to complete a quint or a sextet is a detail our (more in the panel transcript) | The owner_question entry correctly claims nothing, but it defers a rule that three other entries already state as settled: calling-for-exposure ("or a larger (more in the panel transcript) |
| `card-notation` | 2 | "On the card, each color stands for a different suit, not a fixed one" with no carve-out for F, N/E/W/S, and (more in the panel transcript) | No wrong statement, but a scope gap: flowers, winds, and the soap-as-zero are printed in a color on the card yet are not suit tiles, so a reader applying the (more in the panel transcript) |
| `card-notation/source-ref-162` | 1 | source_ref: "Card legend conventions (colors as suits, C and X, F and D, Soap as zero); cross-checked via (more in the panel transcript) | Cited article 162 does not support the cross-check claim, and none of the three cited articles covers C and X or F and D. |
| `charleston-blind-pass` | 1 | "If you do not want to pass three tiles from your own hand, you may take one, two, or all three tiles being (more in the panel transcript) | Rule content is correct, but this sentence tracks the League's own blind-pass sentence (back of the card, panel 1, and MJME 2024 pp.12-13, as quoted on Mahj (more in the panel transcript) |
| `courtesies-vs-rules` | 3 | "Courtesies are local customs a table agrees on, such as how a wall game is paid or whether the same dealer (more in the panel transcript) | The courtesy-pass clause is verified (optional League rule, negotiated, either player may decline). The two examples of 'courtesies', however, are both matters (more in the panel transcript) |
...
| `dead-hand-details/paraphrase` | 1 | "A dead player stops drawing and discarding but still pays the winner of that deal. You do not declare your (more in the panel transcript) | Both sentences track the structure of the Mah Jongg Made Easy passage quoted in Mahj Life 205; low risk because the words are NMJL's, not Mahj Life's, and are (more in the panel transcript) |
| `dead-hand-details/varies-by-house` | 1 | varies_by_house: true; house_note: 'Tables enforce dead hand challenges with different levels of strictness.' | A house flag on a standard League rule risks the model presenting dead-hand law as table-dependent; the League defines both the rule and the challenge (more in the panel transcript) |
| `dead-hand-jokers` | 3 | Whether a joker sitting in a dead player's exposure can still be redeemed depends on which exposure it is in (more in the panel transcript) | Accurate in substance but exceeds the owner_question contract by asserting what the ruling turns on; it also leaves out that the answer additionally depends on (more in the panel transcript) |
| `exposures-basics` | 3 | "Until you discard, you may still fix a mistake in the exposure; after that it is locked, and the only later (more in the panel transcript) | Incomplete locking condition: the entry's own cited source (Mahj Life article 177) and article 6 say an exposure is locked by EITHER a discard OR a joker (more in the panel transcript) |
| `exposures-basics-joker-lock` | 1 | exposures-basics: "Until you discard, you may still fix a mistake in the exposure; after that it is locked" | Same omission as calling-for-exposure: the League also locks an exposure once you make a joker exchange in that turn. Out of my batch but it is the same rule (more in the panel transcript) |
| `exposures-basics-vs-calling-quints-sextets` | 1 | exposures-basics: "Only a Pung, Kong, Quint, or Sextet can be exposed" versus calling-quints-sextets (more in the panel transcript) | Not a contradiction of canonical entries, but the two entries send readers opposite signals on whether a quint or sextet can be called and exposed; worth the (more in the panel transcript) |
| `extra-calling-window-wording-out-of-batch` | 1 | calling-for-mahjong (outside this batch): 'the chance to call ends once the next player has drawn and racked (more in the panel transcript) | Same scope gap as passing-on-a-discard: the League window also closes when the next player discards without racking, as the owner-approved calling-discard (more in the panel transcript) |
| `extra-corpus-stale-vs-head` | 1 | corpus.json entry calling-quints-sextets (source owner_question, classification nmjl_clarification) does not (more in the panel transcript) | The review corpus and the shipping source disagree on one of the seven batch entries; the other six match HEAD byte-for-byte. Reviewers and the owner are not (more in the panel transcript) |
| `extra-exposures-basics-fix-window` | 1 | exposures-basics (outside this batch, same wording as calling-for-exposure): "Until you discard, you may (more in the panel transcript) | Same scope risk as calling-for-exposure: the League also locks an exposure once the player performs a joker exchange, not only on discard. Flagged so the two (more in the panel transcript) |
| `extra-head-calling-quints-provenance` | 1 | HEAD provenance for calling-quints-sextets: 'League rulebook and bulletin rule that a discard may be claimed (more in the panel transcript) | The resolved answer text is correct, but the cited cross-check is weak: Mahj Life article 146 is 'Calling a Discard to Expose a Block with Singles' (a block of (more in the panel transcript) |
| `flowers` | 1 | "Flowers are all interchangeable, and they are not numbered." | The rule statement (all eight flowers interchangeable, any flower stands in for any other) is correct. The physical description "they are not numbered" is the (more in the panel transcript) |
| `flowers-numbering-scope` | 1 | "they are not numbered" | Not a rule error, but a wording/scope risk worth the owner's eye. In League play the flowers are treated as unnumbered and interchangeable (the owner's hard (more in the panel transcript) |
| `joker-exchange-timing-correction-contested` | 1 | "fix it before you discard" | Secondary sources disagree on whether a wrong-tile joker exchange can be corrected at all, so stating it as settled League law risks a wrong statement. This is (more in the panel transcript) |
| `joker-exchange/sequencing-scope` | 1 | "you may, on your own turn, hand over the real tile" (the entry does not say WHEN in the turn) | Not a wrong statement, but a sequencing gap a learner can fall into: the League treats a joker exchange made before the turn has properly started (before you (more in the panel transcript) |
| `jokers-basics` | 2 | "Jokers are wild tiles, and they are unique to American mahjong." | The wild-tile rule content is correct; the "unique to American mahjong" clause is refutable as a factual statement about other variants. It is not a League (more in the panel transcript) |
| `jokers-basics-identical-scope` | 1 | "A joker can stand in for any tile inside a group of 3 or more, meaning a Pung, Kong, Quint, or Sextet." | Correct as written because the "meaning a Pung, Kong, Quint, or Sextet" clause limits it to groups of identical tiles. The risk is scope: this entry's question (more in the panel transcript) |
| `jokers-basics-unique-claim` | 1 | "Jokers are wild tiles, and they are unique to American mahjong." | The clause "unique to American mahjong" is a factual claim about other variants, not a League rule, and it is contradicted by evidence: Vietnamese mahjong sets (more in the panel transcript) |
| `jokers-group-of-three-scope` | 1 | jokers-basics: "any tile inside a group of 3 or more, meaning a Pung, Kong, Quint, or Sextet"; joker-in-pair (more in the panel transcript) | Both entries are correct because each immediately restricts "group of 3 or more" to Pung, Kong, Quint, or Sextet, which are groups of identical tiles. The (more in the panel transcript) |
| `last-tile-of-wall` | 3 | "If the last tile is drawn and no one has declared mahjong, the deal ends as a wall game and no one pays." | Sequencing is one step early. The League's wall-game trigger is that the last tile has been picked AND discarded with no mahjong declared; the entry's own (more in the panel transcript) |
| `mahjong-in-error` | 2 | If you said mahjong but had not yet exposed any tiles, there is no penalty and you keep playing. ... After a (more in the panel transcript) | The first sentence is correct, but the entry is an owner_question entry that states three positive rules, and the final sentence is an unqualified 'always' (more in the panel transcript) |
| `mahjong-in-error-house-note` | 1 | House note: If other players threw in their hands after the false call, ask your table how it settles that (more in the panel transcript) | Internally inconsistent guidance that risks presenting a League-covered matter as a table custom: it tells the reader to ask the table while also saying the (more in the panel transcript) |
| `misnamed-discard` | 3 | Misnaming a discard carries its own League rules, and the consequence depends on whether anyone called the (more in the panel transcript) | Accurate but exceeds the owner_question contract: an owner_question entry must claim nothing beyond saying the instructor is confirming, yet this sentence (more in the panel transcript) |
| `misnamed-discard/tournament-rules-conflict` | 1 | tournament-rules: 'each tournament director adds procedures of their own: timed rounds, a point system, and (more in the panel transcript) | Not a literal contradiction, but read together a player will infer that misnamed-discard penalties are tournament inventions, when the League itself imposes (more in the panel transcript) |
| `naming-discards` | 1 | "When your discard matches the tile discarded just before it, the League accepts saying same as well as (more in the panel transcript) | Facts verified and cross-check supported; second sentence structurally tracks the Mahj Life article 242 commentary line, and the bare word same needs quotation (more in the panel transcript) |
| `open-vs-closed` | 2 | "Closed hands, also called concealed hands, must be built from your own draws, with no calling to build (more in the panel transcript) | The operative rule (no calling for groups; only the mahjong tile may be claimed) is correct and verified. The clause "must be built from your own draws" is (more in the panel transcript) |
| `order-of-play` | 1 | On your turn you either draw the next tile from the wall or call the most recent discard, then you discard (more in the panel transcript) | Every stated fact is correct (East discards first; East, South, West, North to the right/counterclockwise; 13 between turns, 14 during). The wording risk is (more in the panel transcript) |
| `passing-on-a-discard` | 1 | Once the next player has drawn and racked a tile, that discard is out of reach for everyone. | The two rule statements ('you never have to call'; window closes on rack) are correct, but the closing condition is stated as if racking were the only trigger. (more in the panel transcript) |
| `passing-on-a-discard-window` | 1 | passing-on-a-discard: "Once the next player has drawn and racked a tile, that discard is out of reach for (more in the panel transcript) | Same window gap as calling-for-mahjong: the League also closes the window when the next player discards (or picks and requests a joker exchange without (more in the panel transcript) |
| `passing-on-a-discard-window-clause` | 1 | "Once the next player has drawn and racked a tile, that discard is out of reach for everyone." | Not wrong, but narrower than the League rule and narrower than the owner's own house note on calling-discard. The window also closes when the next player (more in the panel transcript) |
| `payments-basics` | 2 | A wall game pays nothing. | All scoring statements are verified League rules (discarder pays double, others pay value; self-pick all pay double; jokerless doubles when the hand could have (more in the panel transcript) |
| `payments-basics-source-ref` | 1 | source_ref: "League payment rules (discarder pays double, self-pick all pay double, jokerless doubles, wall (more in the panel transcript) | Cited articles only partly support the cross-check claim. Article 98 is about two identical tiles discarded in rapid succession and which discarder pays (more in the panel transcript) |
| `picking-ahead` | 1 | if a tile was picked too early, stop and let the table sort it out before anyone else plays. | The prohibition and the 'can make your hand dead' hedge are correct, but 'let the table sort it out' invites house discretion where the League prescribes the (more in the panel transcript) |
...
| `players-count` | 1 | "Many groups adapt it when only 3 can play, but the standard game seats 4." plus house_note: "Adaptations for (more in the panel transcript) | Not a wrong statement (4 is the standard game) and the classification is acceptable, but the framing presents 3-player play as purely a table adaptation when (more in the panel transcript) |
| `players-count-house-note-league-procedure` | 1 | house_note: "Adaptations for 3 players vary from table to table; agree on the format before you start." | Not wrong (tables do vary), but it reads as if the League is silent on three players. The League publishes an official three-handed procedure, so a table that (more in the panel transcript) |
| `rules-source` | 1 | "Find My Mahj answers only from those rules as reviewed by our instructor" | The factual clauses about the League are verified and the entry correctly avoids naming a month; the promise about instructor review is not yet true of this (more in the panel transcript) |
| `source-title-template-on-non-rule-entries` | 1 | source_title "NMJL rule located and cross-checked through secondary research" on tournament-rules (more in the panel transcript) | Metadata mislabels non-rule entries as located NMJL rules, contradicting their own classification and source_ref. |
| `tournament-rules` | 2 | "each tournament director adds procedures of their own: timed rounds, a point system, and penalties for (more in the panel transcript) | Classification tournament_rule is right and the framing (League rules as foundation, director's rules apply only at that event) is verified; the (more in the panel transcript) |
| `two-players-same-tile` | 2 | "Saying hold or wait counts the same as saying call; the word you use does not change who has priority." | Conflation of Mahj Life commentary with League law. The priority rule (mahjong first, then next in turn) is League text, but "hold or wait counts the same as (more in the panel transcript) |
| `two-players-same-tile/paraphrase-264` | 1 | "the word you use does not change who has priority" and "A player who hesitates can lose the tile once (more in the panel transcript) | Closest paraphrase in the batch. Each sentence maps one-to-one onto a Mahj Life commentary sentence in article 264 (word choice does not affect priority (more in the panel transcript) |
| `wall-game` | 1 | house_note: "Tables differ on whether the same dealer deals again after a wall game." with varies_by_house (more in the panel transcript) | The answer text itself is correct (no winner, no payment) and correctly classified. The flag is on the note: the League is not silent on who is East after a (more in the panel transcript) |
| `wall-game-house-note` | 1 | TS house_note on the wall-game entry (lib/rules/knowledge.ts, about line 586): "Tables differ on whether the (more in the panel transcript) | Not in the corpus answer, but it is served alongside it and shares the same scope risk as the courtesies entry: it is true that tables differ, yet the League's (more in the panel transcript) |
| `wall-game-house-note-dealer-default` | 1 | house_note: "Tables differ on whether the same dealer deals again after a wall game." | Not wrong, but scope risk: the note gives no League default, and the League does have one. Under NMJL rules the deal moves to the right after a wall game just (more in the panel transcript) |
| `wall-game-vs-last-tile-of-wall` | 1 | wall-game: "If the wall runs out of tiles before anyone declares mahjong, the hand ends with no winner." (more in the panel transcript) | Not a true contradiction under the League definition (a wall game exists only once all tiles have been picked and discarded with no mahjong), but the (more in the panel transcript) |
| `xref-quint-sextet-exposure` | 1 | Cross-corpus (outside my batch, touching jokers-basics): exposures-basics says 'Only a Pung, Kong, Quint, or (more in the panel transcript) | My batch entries (jokers-basics, joker-in-pair) list Quint and Sextet as joker-eligible groups and are consistent with all three. But among those three entries (more in the panel transcript) |
