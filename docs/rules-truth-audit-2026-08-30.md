...
- tests/rules-heldout.spec.ts: 82 blind cases (77 at the first, tuning-free run; 82 after the gate rounds added phrasings), written after the routing logic and the
...
| `calling-discard/corpus-missing-house-note` | 1 | corpus.json entry "calling-discard" has no house_note field, but /Users/shaunabruckman/Projects/findmymahjgame-rules/lib/rules/knowledge.ts lines 503-504 attach one that the router serves. | Regenerate corpus.json from knowledge.ts including house_note so every served sentence is inside the review set. |
