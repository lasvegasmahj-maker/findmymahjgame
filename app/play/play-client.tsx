    ...
    return shell(
      <>
          ...
          {games.map((g) => {
            ...
            return (
              <Link key={g.share_code} href={`/t/${g.share_code}`} style={{ display: "block", background: "white", border: "2px solid var(--border)", borderRadius: 16, padding: "1.3rem", textDecoration: "none" }}>
                ...
                <div style={{ marginTop: "0.8rem", color: "var(--pink-text)", fontWeight: 800, fontSize: "1.1rem" }}>Join this game &rarr;</div>
              </Link>
            );
          })}
        </div>
        <div style={{ marginTop: "1.6rem", textAlign: "center" }}>
          ...
          <Link href="/start" style={{ display: "inline-block", color: "var(--pink-text)", fontWeight: 800, fontSize: "1.15rem", margin: "0.4rem 0" }}>Start your own table &rarr;</Link><br />
          <button type="button" onClick={() => { setCaptureWhy("notify"); setStep("capture"); }} style={{ background: "none", border: "none", color: "var(--pink-text)", fontWeight: 700, fontSize: "1.05rem", cursor: "pointer", textDecoration: "underline" }}>Or tell us when a new game opens</button>
        </div>
      </>
    );
  }
