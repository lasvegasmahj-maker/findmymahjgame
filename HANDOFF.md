...
- SQL schema lives in `supabase/migration.sql`, but it is incomplete: it creates and enables RLS on `player_listings`, `venue_listings`, `event_listings`, `ad_listings`, `inquiries`, and `admins`, but does NOT include `listing_submissions` or `promo_codes` (both added ad hoc later). `listing_submissions` therefore likely has no RLS. Bring the migration file up to date with every table and its policies, including Storage bucket policies.
...
- FIXED (2026-06-07), security: admin auth is now server-side. The dashboard no longer ships a password or uses the anon key. Login goes through `/api/admin/login` (server-side password check, HMAC HttpOnly SameSite=Strict cookie), and all reads and writes go through cookie-gated service-role routes (`/api/admin/data`, `/api/admin/update`). `NEXT_PUBLIC_ADMIN_PASSWORD` was removed from the code and Vercel; a server-only `ADMIN_PASSWORD` replaced it and was rotated.
- CONFIRMED OK / PARTIAL, security: RLS. Verified by live anon probes: `inquiries` returns zero rows to the anon key (RLS on, insert-only, no read), and `player_listings`, `venue_listings`, `event_listings`, `ad_listings` expose only `status='published'` rows. So there is no live PII breach via those tables. REMAINING: `listing_submissions` is not in `supabase/migration.sql` and likely has no RLS (it currently reads empty, so the probe was inconclusive). Run the hardening SQL (see security report) to enable RLS on it with no anon policy before it accumulates advertiser PII.
- HIGH, security (storage): the `logos` Supabase Storage bucket allowed anonymous upload and listing. FIXED in code (uploads now go through the server route `/api/upload-logo` with the service-role key, and the anon write path was removed from both forms). Still REQUIRES the bucket policy SQL to be run so the anon key loses direct write/list at the database level.
