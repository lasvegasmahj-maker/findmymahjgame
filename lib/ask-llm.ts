...
// Optional model refinement for directory intent extraction. Ships dormant: without
// ANTHROPIC_API_KEY every question goes through the deterministic parser, and the site behaves
// identically. The model never sees the database and never produces answer text; it returns
// only a filter object that must pass validateIntent, so it cannot invent a fact or widen a
// query beyond what deterministic search supports. Rules answers are never rephrased: the
// shared core's model contract (lib/ask-core/model) frames approved text and nothing else.
