import { normalizeQuestion } from "@/lib/ask-core/index.ts";
import { DAY_NAMES, type DayName, type TimeOfDay } from "@/lib/schedule";
import { RADIUS_OPTIONS, type RadiusMiles } from "@/lib/geo";

// Deterministic intent extraction for the directory half of Ask Find My Mahj. This is the
// primary parser, not a fallback: every factual filter must be derivable from the question
// text by rules, so the assistant works identically with or without a model. When
// ANTHROPIC_API_KEY is present, lib/ask-llm.ts may refine the same shape, but its output
// passes through validateIntent and can never widen beyond what deterministic search supports.
// Whether a question is a rules question at all is decided by the shared core (classifyTopic).
